...
void ubug_tran_db(void *pInfo, void *uData, char *pUrl, int uLen)
{
	...
	sprintf(buff_place_start(webInfo->w_buff), TRAN_URL_BEG, urlTabName, uLen, pUrl,
		pData->ud_poff, pData->ud_foff, webInfo->w_pattern) : 
	sprintf(buff_place_end(webInfo->w_buff), TRAN_URL, uLen, pUrl,
		pData->ud_poff, pData->ud_foff, webInfo->w_pattern)));
}
