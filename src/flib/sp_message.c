...
#define	sp_msg_is_part_exist(nPart) \
	((nPart < PART_MAX && \
    (nPart == PART_URLBUG || nPart == PART_TEXTBUG || nPart == PART_EXTBUG)) ? \
...
/*-----sp_msg_frame_init-----*/
MSGSET *sp_msg_frame_init(
        char *mName, int mNum, int mFd, int mSleep,
        msctim fCtime, mskpwk fKwork, mssend fSend)
{
	MSGSET	*msgSet;
...
int sp_msg_frame_run(MSGSET *msgSet, void *kpPara)
{
    ...
    while (sp_msg_select_read(
           msgSet->ms_fd, &readMsg, msgSet->ms_sptime) > FUN_RUN_END) {
        if (sp_msg_exam_recver(
            &readMsg, msgSet->ms_num, msgSet->ms_name) == FRET_P)
